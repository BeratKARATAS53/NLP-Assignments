...
def processDataUnigram(data): # Process every single line by 're' library
    data = data.lower()
    data = data.replace("|", " ")
    data = re.sub(r'[^A-Za-z. _-]', '', data)
...
def prob(sentence):
    ...
    #     bigramCounts[listToString] = sum([data.count(listToString) for data in bigram_list])
    ...
    #     listToString = ' '.join([str(elem) for elem in temp])
    #     trigramCounts[listToString] = sum([data.count(listToString) for data in trigram_list])
...
dataset_list = dataset("cbt_train.txt")
...
# generate_list["Unigram"] = ['bears made and was of tell change a he beautifully the tender that love advance place <s> suit . my','rosy so they the round the papa was said too bed about drinking see </s>','and frequently <s> a to rye a asked go animals have eyes <s> </s>','them over is <s> the brimming the to things the conduct of <s> time off the kind marry to from','to gasped day flitting <s> . took and am hands if in nothing to needle made the wire straw gay']
# generate_list["Bigram"] = ['when mrs. bhaer nodded . </s>','and rest he longed for a sad enough for her that i believe what you keep the never-tiring mail .','and we stood craning its sordid surroundings at him . </s>','his scouts wriggle of about his shins as long lawsuit that made a strip of the scrubbing the thought of','all round his high above them . </s>']
# generate_list["Trigram"] = ['<s> i ve gone and i should like to be asleep and had forgotten to wind yarn wash the cups','<s> like all kindred spirits just as well as everyone else despaired i never did . </s>','<s> not those which hallow your bridal-night </s>','<s> i profess not to be an end so why should we wish to see eden . </s>','<s> into bowser s little kingdom of iolchos he found himself in a voice from the wall and so tired that']
